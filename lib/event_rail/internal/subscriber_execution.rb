module EventRail
  module Internal
    ...
    # delivered event as its cause, and identity, so the follow-up derives the same
    # ID on every delivery of that cause rather than a new one per delivery job.
        ...
        def __event_rail_delivered_event__
          ...
          unless arguments.length == 1 && expected.any? { |event_class| candidate.instance_of?(event_class) }
            raise UnexpectedEventError.new(
              "#{self.class} handles #{expected.map(&:to_s).sort.join(", ")} but received " \
              "#{describe_arguments}",
          ...
          # logical message or to scope identity on. Rejecting only by class would let
          # one through, because a proposal is an instance of the declared class.
